...
module.exports = async (client) => {
    db.serialize(() => {
        ...
        async function startFarming(userId, fumoName, coinsPerMin, gemsPerMin) {
            ...
            const row = await db.getAsync(`SELECT quantity FROM farmingFumos WHERE userId = ? AND fumoName = ?`, [userId, fumoName]);
            ...
            const intervalId = setInterval(async () => {
                ...
                // --- BOOST LOGIC: apply boost multipliers here ---
                const { coinMultiplier, gemMultiplier, coinSources, gemSources, boosts } = await getActiveBoostMultipliers(userId);
                ...
                // --- Console log for boosts and rewards ---
                // if (boosts && boosts.length > 0) {
                //     console.log(`[Farming Boosts] User: ${userId} | Fumo: ${fumoName} | Boosts:`, boosts.map(b => ({
                //     type: b.type,
                //     source: b.source,
                ...
                //     })));
                // }
                // console.log(`[Farming Reward] User: ${userId} | Fumo: ${fumoName} | Coins: ${coinsAwarded} | Gems: ${gemsAwarded} | CoinMultiplier: ${coinMultiplier} | GemMultiplier: ${gemMultiplier}`);
                if (coinSources.length > 0 || gemSources.length > 0) {
                    // console.log(`[Boost Sources] Coins: ${coinSources.join(', ')} | Gems: ${gemSources.join(', ')}`);
                }
                // console.log(`+${coinsAwarded} coins, +${gemsAwarded} gems → ${fumoName} (x${quantity})`);
                } catch (err) {
