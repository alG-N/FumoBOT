...
const { formatTimeRemaining } = require('./timeUtils');
...
// formatTimeRemaining for codes (wraps the general one for expiry timestamps)
function formatCodeTimeRemaining(expiresAt) {
    const now = Date.now();
    const expiry = new Date(expiresAt).getTime();
    const diff = expiry - now;
    
    if (diff <= 0) return 'Expired';
    return formatTimeRemaining(diff) || '< 1m';
}
...
module.exports = {
    ...
    formatCodeStats,
    formatTimeRemaining: formatCodeTimeRemaining,
    getCategoryColor
};